...
HWLOC_PLUGINS_PATH=${HWLOC_top_builddir}/hwlo
...
(
  ...
  echo
  $calc --if synthetic --input "numa:2 cache:2 pu:1" numa:0 cache:3
  $calc --if synthetic --input "group:2 numa:2 cache:2 cache:2 pu:1" Group1:1 NUMA:0
  $calc --if synthetic --input "group:2 numa:2 cache:2 cache:2 pu:1" l2:0-2 L1cache:13:3
  echo
...
diff -u $srcdir/test-hwloc-calc.output "$file"
rm -rf "$tmp"
